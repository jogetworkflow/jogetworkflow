...
<jnlp codebase="${serverPath}/webstart">
    ...
    <resources>
        <j2se version="1.5+"/>
        <jar href="wflow-designer.jar" version="2.0-SNAPSHOT" main="true"/>
    ...
    <application-desc main-class="org.joget.designer.Designer">
        ...
        <c:if test="${!empty param.deploy}">
            <argument>deploy:${param.deploy}</argument>
        </c:if>
        <c:if test="${!empty param.update}">
            <argument>update:${param.update}</argument>
        </c:if>
