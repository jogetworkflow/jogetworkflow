...
<div id="loading_${id}" style="display:none">
    <center><img src="${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}${pageContext.request.contextPath}/images/joget/portlet_loading.gif"/></center>
</div>
    ...
    function doInboxLogin_${id}(){
        ...
        var loginCallback_${id} = {
            success: function(){
                ...
                if(${login} ==true){
                    url = '/web/js/workflow/processList.js?';
                }else{
                    url = '/web/js/workflow/inbox.js?';
                }
...
<form id="loginForm_${id}" name="loginForm" action="javascript: doInboxLogin_${id}();" method="POST">
    <table>
        <tr><td><fmt:message key="general.login.label.username" />: </td><td><input type='text' id='j_username_${id}' name='j_username_${id}' value='<c:if test="${not empty param.login_error}"><c:out value="${SPRING_SECURITY_LAST_USERNAME}"/></c:if>'/></td></tr>
        <tr><td><fmt:message key="general.login.label.password" />:</td><td><input type='password' id='j_password_${id}' name='j_password_${id}'></td></tr>
    </table>
