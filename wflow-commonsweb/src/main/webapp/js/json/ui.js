...
PopupDialog.prototype = {
  ...
  show: function(){
      ...
      if (this.windowName) {
          this.popupWindow = window.open(this.src, this.windowName, "width=" + this.width + ",height=" + this.height + ",resizable=1,scrollbars=1,top=50,left=50");
          return;
      }
      var thisObject = this;
      ...
      var openDialog = function() {
          var newFrame = document.getElementById("jqueryDialogFrame");
          if (newFrame != null) {
              setTimeout(function() { newFrame.setAttribute("src", newSrc); }, 500);
          }
      }
    ...
    getSelectedRows: function() {
        ...
        if (this.checkbox && this.checkboxSelectSingle) {
            ...
            for (i=0; i<rows.length; i++) {
                var id = rows[i].id.substring(3); // strip prefix "row"
                selected.push(id);
            }
...
TabView.prototype = {
    ...
    init : function(){
        ...
        this.tabView = $("#" + this.div + " > ul").tabs({ fx: { opacity: 'toggle', duration: 'fast' } });
    },
